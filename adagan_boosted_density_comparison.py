...
def main():
    ...
    opts['dataset'] = 'gmm' #'gmm' # gmm, circle_gmm,  mnist, mnist3, cifar ...
    ...
    opts["plot_every"] = 700 # set -1 to run normally
    opts["eval_points_num"] = 10000
    ...
    for run in range(opts["number_of_runs"]):
        logging.info('Beginning run {} of {}'.format(run, opts["number_of_runs"]))
        opts['random_seed'] += 1
